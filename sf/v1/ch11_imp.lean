...
/-
trying to use builtins since sf doesn't import other chapters
-/
...
/-
TODO: not use these earlier (gross)
-/
...
theorem silly₁ (ae) : aeval ae = aeval ae :=
by try { reflexivity, }

theorem silly₂ (P : Prop) (hp : P) : P :=
begin
  try { reflexivity, },
  exact hp,
end
...
/- even this is annoyingly repetitive -/
...
begin
  induction a; try {
    ...
    refl,
  cases a_a₁; try {
    simp only [optimize_0plus, aeval, *] at *,
  },
  cases a_a₁; simp only [optimize_0plus, aeval, zero_add, *],
end
...
/- all goals can be a bit nicer for names -/

theorem optimize_0plus_sound''' (a)
  : aeval (optimize_0plus a) = aeval a :=
begin
  induction a,
  all_goals { simp only [optimize_0plus, aeval, *], },
  case APlus : a₁ a₂ ih₁ ih₂ {
    cases a₁,
    all_goals { simp only [optimize_0plus, aeval, *] at *, },
    case ANum : {
      cases a₁; simp only [optimize_0plus, aeval, zero_add, *],
    },
  },
end
...
begin
  induction b;
  simp only [
    optimize_0plus_b, beval, *, bool.to_bool_eq,
    optimize_0plus_sound ],
end

/-
TODO (but probably not) optimize
-/
...
meta def simpl_and_try {α : Type} (c : tactic α) : tactic unit := do
  ...
  /- unlike coq, simp can fail -/
  try $ simp_target s,
  try c

meta def simpl_and_try' {α : Type} (c : tactic α) : tactic unit :=
  simp_lemmas.mk_default >>= λs, try $ simp_target s >> try c

example : 5 + 1 ≠ 2 + 3 := begin
  simpl_and_try (omega_nat ff)
end
...
/-
TODO: this is mentioned in tactic_writing in lean community
doesn't seem to work though
-/
-- run_cmd add_interactive [`simpl_and_try]

end tactic

namespace tactic.interactive

meta def simpl_and_try (c : itactic) : tactic unit :=
tactic.simpl_and_try c
...
lemma omega_test (m n o p) (h : m + n ≤ n + o ∧ o + 3 = p + 3)
  : m ≤ p :=
begin
  cases h with hl hr,
  have : o = p, injections,
...
/- hard disagree about preferring previous style -/
inductive aevalR : aexp → ℕ → Prop
| E_ANum (n) : aevalR (ANum n) n
...
/- no idea why level changes in the sf book -/
reserve infixl ` \\ `:25
...
/- i don't see a way to use reserved notation in a lean definition -/
inductive aevalR : aexp → ℕ → Prop
| E_ANum (n) : aevalR (ANum n) n
| E_APlus (e₁ e₂) (n₁ n₂) :
  aevalR e₁ n₁ → aevalR e₂ n₂ → aevalR (APlus e₁ e₂) (n₁ + n₂)
...
begin
  split,
    ...
    case E_APlus : e₁ e₂ n₁ n₂ h₁ h₂ ih₁ ih₂ {
      rw [ih₁, ih₂],
    },
    case E_AMinus : e₁ e₂ n₁ n₂ h₁ h₂ ih₁ ih₂ {
      rw [ih₁, ih₂],
    },
    case E_AMult : e₁ e₂ n₁ n₂ h₁ h₂ ih₁ ih₂ {
      rw [ih₁, ih₂],
    },
  intro h,
  /- subst breaks case labels -/
...
begin
  split,
    intro h; induction h; simp [aeval, *],
  intro h; induction a generalizing n; subst h; constructor; simp *,
end
...
begin
  split; intro h,
    induction h; simp [aeval_iff_aevalR, beval, *] at *,
  induction b generalizing bv; subst h; constructor; simp [aeval_iff_aevalR, *],
end
...
/- still unaware of any scope things -/

/-
for whatever reason, need zero, one, and add instances
...
/-
don't use notation on =
it breaks generalize and
requires type annotation everywhere else
-/
infix ` == ` := BEq
...
/- true by default. also not super helpful -/
...
notation x ` !→ `:10 v:50 ` ; ` m := t_update m x v
notation x ` !→ `:10 v := t_update empty_st x v
...
notation `TEST ` c₁ ` THEN ` c₂ ` ELSE ` c₃ ` FI` := CIf c₁ c₂ c₃
...
notation `FOR ` intr ` ;;; ` cond ` ;;; ` up ` DO ` body ` END` :=
...
| SKIP := tt
| (_ ::= _) := tt
| (c₁ ;; c₂) := no_whiles c₁ && no_whiles c₂
| (TEST _ THEN ct ELSE cf FI) := no_whiles ct && no_whiles cf
| (WHILE _ DO _ END) := ff
...
theorem no_while_eqv (c)
  : no_whiles c = tt ↔ no_whilesR c :=
begin
  split,
    intro h,
    induction c,
...
/- honestly, not sure about short circuit behavior in general here -/
...
/-
these should all be easy to prove
...
/- way more tedious than enlightening -/
/-
TODO - fixup naming
-/
...
--   : st₁ = st₂ ∧ s₁ = s₂ :=
-- begin
--   induction c generalizing st st₁ st₂ s₁ s₂,
--   case CSkip {
--     cases hc₁,
--     cases hc₂,
--     split; refl,
